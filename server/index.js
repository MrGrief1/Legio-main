...
app.post('/api/polls/:id/resolve', authenticateToken, requireCreatorOrAdmin, async (req, res) => {
    ...
    try {
        ...
        // Завершение начисляет баллы и отменить его нечем, поэтому обычный клик по варианту не
        // закрывает опрос, пока идёт голосование: без этой проверки один промах мышью закрывал
        // опрос, до конца которого оставались месяцы, — так за август закрылись десять опросов со
        // сроками вплоть до 2027 года. Досрочное завершение при этом возможно, но только осознанно:
